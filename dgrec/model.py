...
class DGRec(object):
    ...
    def __init__(self, args, support_sizes, placeholders):
        ...
        self.mask = v1.cast(self.mask_y, dtype=tf.float32)
        self.point_count = v1.reduce_sum(self.mask)
        ...
        self.dense_layers = []
        self.loss = 0
        self.global_step = v1.Variable(0, name='global_step', trainable=False)
        self.lr = v1.maximum(1e-5, v1.train.exponential_decay(args.learning_rate,
                                                            self.global_step,
                                                            args.decay_steps,
                                                            args.decay_rate,
                                                            staircase=True))
        self.optimizer = v1.train.AdamOptimizer(learning_rate=self.lr)
        self.build()

    def global_features(self):
        self.user_embedding = v1.get_variable('user_embedding', [self.n_users, self.emb_user],\
                                        initializer=v1.glorot_uniform_initializer())
        feature_layer1 = v1.nn.embedding_lookup(self.user_embedding, self.support_nodes_layer1)
        feature_layer2 = v1.nn.embedding_lookup(self.user_embedding, self.support_nodes_layer2)
    ...
    def local_features(self):
        ...
        inputs_1 = v1.nn.embedding_lookup(self.embedding, self.support_sessions_layer1)
        inputs_2 = v1.nn.embedding_lookup(self.embedding, self.support_sessions_layer2)
        outputs1, states1 = v1.nn.dynamic_rnn(cell=self.lstm_cell,
                                            inputs=inputs_1, 
                                            sequence_length=self.support_lengths_layer1,
                                            initial_state=initial_state_layer1,
                                            dtype=tf.float32)
        outputs2, states2 = v1.nn.dynamic_rnn(cell=self.lstm_cell,
                                            inputs=inputs_2, 
    ...
    def global_and_local_features(self):
        ...
        global_local_layer2 = v1.concat([global_feature_layer2, local_feature_layer2], -1)
        global_local_layer1 = v1.concat([global_feature_layer1, local_feature_layer1], -1)
        ...
        for layer in range(len(num_samples)):
            ...
            for hop in range(len(num_samples) - layer):
                ...
                h = aggregator((hidden[hop],
                                v1.reshape(hidden[hop + 1], neigh_dims)))
                next_hidden.append(h)
            hidden = next_hidden
        return hidden[0], aggregators

    def decode(self):
        self.lstm_cell = lstm_cell = v1.nn.rnn_cell.BasicLSTMCell(self.hidden_size)
        initial_state = lstm_cell.zero_state(self.batch_size, dtype=tf.float32)
        time_major_x = v1.transpose(self.input_x)
        inputs = v1.nn.embedding_lookup(self.embedding, time_major_x)
        outputs, state = v1.nn.dynamic_rnn(cell=lstm_cell,
                                            inputs=inputs, 
        ...
        slices = v1.split(outputs, num_or_size_splits=self.max_length, axis=0)
        return [v1.squeeze(t,[0]) for t in slices]
    ...
    def step_by_step(self, features_0, features_1_2, dims, num_samples, support_sizes, 
        ...
        return v1.stack(outputs, axis=0)

    def build(self):
        v1.disable_v2_behavior()
        self.embedding = embedding = v1.get_variable('item_embedding', [self.n_items, self.emb_item],\
                                        initializer=v1.glorot_uniform_initializer())
        ...
        outputs = self.step_by_step(features_0, features_1_2, self.dims, self.num_samples, self.support_sizes,
                                concat=self.concat)
        concat_self = v1.concat([features_0, outputs], axis=-1)

        # exchange first two dimensions.
        self.transposed_outputs = v1.transpose(concat_self, [1,0,2])

        self.loss = self._loss()
        self.sum_recall = self._recall()
        self.sum_ndcg = self._ndcg()
        # -------------------------------------------------------
        # user_output = short term interest + long term interest, (200, 1)
        self.user_output = self.transposed_outputs
        # -------------------------------------------------------
        grads_and_vars = self.optimizer.compute_gradients(self.loss)
        clipped_grads_and_vars = [(v1.clip_by_value(grad, -5.0, 5.0) if grad is not None else None, var)
    ...
    def _loss(self):
        ...
        self.logits = logits = v1.matmul(fc_layer(v1.reshape(self.transposed_outputs, [-1, self.dim2+self.hidden_size])), self.embedding, transpose_b=True)
        for dense_layer in self.dense_layers:
            for var in dense_layer.vars.values():
                reg_loss += self.weight_decay * v1.nn.l2_loss(var)
        for aggregator in self.aggregators:
            for var in aggregator.vars.values():
                reg_loss += self.weight_decay * v1.nn.l2_loss(var)
        reshaped_logits = v1.reshape(logits, [self.batch_size, self.max_length, self.n_items])
        xe_loss += v1.nn.sparse_softmax_cross_entropy_with_logits(labels=self.input_y,
                                                            logits=reshaped_logits,
                                                            name='softmax_loss')
        xe_loss *= self.mask
        return v1.reduce_sum(xe_loss) / self.point_count + reg_loss

    def _ndcg(self):
        predictions = v1.transpose(self.logits)
        targets = v1.reshape(self.input_y, [-1])
        pred_values = v1.expand_dims(v1.diag_part(v1.nn.embedding_lookup(predictions, targets)), -1)
        tile_pred_values = v1.tile(pred_values, [1, self.n_items-1])
        ranks = v1.reduce_sum(v1.cast(self.logits[:,1:] > tile_pred_values, dtype=tf.float32), -1) + 1
        ndcg = 1. / (log2(1.0 + ranks))
        mask = v1.reshape(self.mask, [-1])
        ndcg *= mask
        return v1.reduce_sum(ndcg)

    def _recall(self):
        predictions = self.logits
        targets = v1.reshape(self.input_y, [-1])
        recall_at_k = v1.nn.in_top_k(predictions, targets, k=20)
        recall_at_k = v1.cast(recall_at_k, dtype=tf.float32)
        mask = v1.reshape(self.mask, [-1])
        recall_at_k *= mask
        return v1.reduce_sum(recall_at_k)

def log2(x):
    numerator = v1.log(x)
    denominator = v1.log(v1.constant(2, dtype=numerator.dtype))
