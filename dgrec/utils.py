...
# def load_community_user(data_path):
#     community_history = pd.read_csv(data_path + '/community_user_history.csv')
#     user_id_map = load_map(data_path, 'user')
#     community_history['user_id'] = community_history['user_id'].apply(lambda x:user_id_map[str(x)])
#     return community_history.loc[:, 'user_id'].unique()
