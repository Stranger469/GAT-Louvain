...
def evaluate(sess, model, minibatch, file, val_or_test='val'):
    epoch_val_cost = []
    epoch_val_recall = []
    epoch_val_ndcg = []
    epoch_val_point = []
    ...
    while not minibatch.end_val(val_or_test):
        ...
        outs = sess.run([model.loss, model.sum_recall, model.sum_ndcg, model.point_count, model.user_output], feed_dict=feed_dict)
        epoch_val_cost.append(outs[0])
        epoch_val_recall.append(outs[1])
        epoch_val_ndcg.append(outs[2])
        epoch_val_point.append(outs[3])
        # --------------------------------------
        # print(minibatch.nodes, flush=True)
        # print('user_feature_vector', outs[4], flush=True)
        # TODO: 减少写入数量
        # if minibatch.nodes[0] in community_user:
        #     print(minibatch.nodes[0])
        file.write('{}\t{}\n'.format(minibatch.nodes[0], np.mean(outs[4][0], axis=0).tolist()))
        # user_features[minibatch.nodes[0]] = user_features.get(minibatch.nodes[0], [])
        # user_features[minibatch.nodes[0]].append(outs[4])
        # --------------------------------------
    return [np.mean(epoch_val_cost), np.sum(epoch_val_recall) / np.sum(epoch_val_point), np.sum(epoch_val_ndcg) / np.sum(epoch_val_point), epoch_val_recall, epoch_val_ndcg, input_str, user_features]
...
def test(args, name, minibatch):
    ...
    ckpt = v1.train.get_checkpoint_state(args.ckpt_dir)
    if ckpt and ckpt.model_checkpoint_path:
        print(ckpt.model_checkpoint_path)
        saver.restore(sess, ckpt.model_checkpoint_path)
        print('Restore model from {}!'.format(args.ckpt_dir), flush=True)
    else:
        print('Failed to restore model from {}'.format(args.ckpt_dir), flush=True)
        sys.exit(0)
    
    with open('social_rm_userfeature.csv', 'w') as f:
        ret = evaluate(sess, dgrec, minibatch, f, "calculate")
    ...
    with open('social_rm_result.txt', 'a+') as f:
        f.write(
            "Test results(batch_size=1) for {}:\tloss={:.5f}\trecall@20={:.5f}\tndcg={:.5f}\n".format(name, ret[0], ret[1], ret[2])
        )
    recall = ret[-3]
    ndcg = ret[-2]
    x_strs = ret[-1]
...
def parseArgs():
    ...
    args.dataset = new_args.dataset
    # windows下文件名过长会导致无法删除, 此处进行文件名的精简
...
if __name__ == '__main__':
    ...
    if(args.dataset == 0):
        ...
        data_name = 'movie'
        # test(args, data, 'movie', minibatch)
    elif(args.dataset == 1):
        data_name = 'music'
        # test(args, data, 'music', minibatch)
    elif(args.dataset == 2):
        data_name = 'book'
        # test(args, data, 'book', minibatch)
    elif(args.dataset == 3):
        data_name = 'yelp'
        # test(args, data, 'yelp', minibatch)
    else:
        print('Unknown dataset code {}'.format(args.dataset), flush=True)
        sys.exit(0)
    data = load_data(data_path + data_name)
    print("Done loading {} data..".format(data_name), flush=True)
